/* ————————————————————————————————————————
   The Growing Tree — procedural growth + scroll choreography
...
(function () {
    ...
    function generate(rand) {
        var paths = [];   // {d, w, kind, t0, dur, len}
        var leaves = [];  // {x, y, r, deg, young, t0}
        var trunkPts = []; // skeleton points near the trunk for ring markers
        var count = 0;
        ...
        function branch(x, y, angle, len, w, depth, kind, t0) {
            ...
            // gentle curve via a control point offset along the normal
            ...
            if (kind === 'branch' && depth >= MAX_DEPTH - 1) {
                trunkPts.push({ x: x2, y: y2 });
            }
            ...
            for (var i = 0; i < kids; i++) {
                ...
                // bias growth back toward upright
                childAngle += (-Math.PI / 2 - childAngle) * 0.14;
                branch(x2, y2, childAngle,
                    len * (0.66 + rand() * 0.16),
                    Math.max(0.9, w * 0.68),
                    depth - 1, 'branch', childT0);
            }
        }
        ...
        function root(x, y, angle, len, w, depth, t0) {
            ...
            for (var i = 0; i < kids; i++) {
                ...
                childAngle += (Math.PI / 2 - childAngle) * 0.18; // bias downward
        ...
        branch(BASE_X, BASE_Y, -Math.PI / 2 + (rand() - 0.5) * 0.12,
            130 + rand() * 45, 15, MAX_DEPTH, 'branch', 0.19);
    ...
    var RING_JOBS = [
        { beat: 'beat-wilo',  t0: 0.40 },
        { beat: 'beat-tud',   t0: 0.56 },
        { beat: 'beat-deriv', t0: 0.72 }
    ];

    var BEATS = [
        { id: 'beat-about',     a: 0.02, b: 0.06, c: 0.13, d: 0.18 },
        { id: 'beat-education', a: 0.16, b: 0.20, c: 0.27, d: 0.32 },
        { id: 'beat-wilo',      a: 0.38, b: 0.42, c: 0.50, d: 0.545 },
        { id: 'beat-tud',       a: 0.54, b: 0.58, c: 0.66, d: 0.705 },
        { id: 'beat-deriv',     a: 0.70, b: 0.74, c: 0.85, d: 0.895 },
        { id: 'beat-canopy',    a: 0.90, b: 0.94, c: 1.0,  d: 2.0 }
    ];

    var renderItems = [];  // paths: {el, len, t0, dur}
    var renderLeaves = []; // {el, t0}
    var renderRings = [];  // {el, t0}
    ...
    function build(seed) {
        ...
        // ring markers along the trunk, low → high (early career → present)
        var pts = data.trunkPts;
        if (pts.length >= 3) {
            var picks = [0.18, 0.5, 0.82].map(function (f) {
                return pts[Math.min(pts.length - 1, Math.floor(f * pts.length))];
            });
            picks.forEach(function (pt, i) {
                var g = document.createElementNS(NS, 'g');
                var outer = document.createElementNS(NS, 'circle');
                outer.setAttribute('cx', pt.x); outer.setAttribute('cy', pt.y);
    ...
    function render(p) {
        ...
        for (i = 0; i < renderLeaves.length; i++) {
            ...
            var s = f < 1 ? f * (1.15 - 0.15 * f) : 1; // slight overshoot
        ...
        for (i = 0; i < beatEls.length; i++) {
            ...
            b.el.style.transform = 'translateY(calc(-50% + ' + ((1 - v) * 24).toFixed(1) + 'px))';
        }
    }
    ...
    document.querySelectorAll('.nav-links a[data-goto]').forEach(function (link) {
        link.addEventListener('click', function (e) {
            ...
            var y = st.start + f * (st.end - st.start);
